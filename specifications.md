...
body, app-container, card の基本レイアウトと配色
...
- syncColumnWidths（列幅コピー）・syncFixedColumns（固定列leftオフセット算出）は、getBoundingClientRect().width の小数px値をそのまま使わず Math.round() で整数pxに丸めてから適用する（2026-07 修正。小数のまま body 側テーブルと sticky 側テーブルという独立した2つのテーブルにそれぞれ適用すると、ブラウザ内部の描画上の丸め処理がテーブルごとに独立して働き、heuristicsモードのようにheuristics技術指標セル等で列数が多く各列幅も30〜48px台と狭い場合に、わずかなズレがスクロール方向に蓄積してヘッダ列とボディ列の境界が視覚的にずれることがあった。600px超（横向きスマホ含む）ではこれらの列幅縮小ルールが適用されず列幅に余裕があるため、症状が確認されていなかった）
