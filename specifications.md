...
- syncColumnWidths（列幅コピー）・syncFixedColumns（固定列leftオフセット算出）は、getBoundingClientRect().width の小数px値をそのまま使わず Math.round() で整数pxに丸めてから適用する（2026-07 修正。小数のまま body 側テーブルと sticky 側テーブルという独立した2つのテーブルにそれぞれ適用すると、ブラウザ内部の描画上の丸め処理がテーブルごとに独立して働き、heuristicsモードのようにheuristics技術指標セル等で列数が多く各列幅も30〜48px台と狭い場合に、わずかなズレがスクロール方向に蓄積してヘッダ列とボディ列の境界が視覚的にずれることがあった。600px超（横向きスマホ含む）ではこれらの列幅縮小ルールが適用されず列幅に余裕があるため、症状が確認されていなかった。※この整数px丸め自体は妥当な対策だが、単独では不十分だった。真因は table-layout: auto かつテーブル幅が制約されていない状況で、見出しテキストの「必要とする幅（preferred width）」自体が大きく、body側・sticky側の2つの独立したテーブルがそれぞれ異なる幅に広がっていたことであり（overflow: hiddenの追加は効果がなく撤回した）、2026-07 に <br class="mobile-line-break"> による強制改行でテキストの必要幅自体を縮めたことで根本的に改善を図った。詳細は controls.responsive のトレンド・スコア列の項を参照）
