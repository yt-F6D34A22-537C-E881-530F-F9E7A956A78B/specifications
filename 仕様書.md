...
iOSの「ホーム画面に追加」用アイコン（<link rel="apple-touch-icon" sizes="180x180">）。180x180px・透過なし（フルブリードの正方形）のPNG。角丸のマスキングはiOS側が自動で行う想定のため、画像側では角丸を持たせない。

#### `history`

2026-07 追加。旧仕様では favicon.ico（最大でも64x64までしか解像度を持たない、かつ角丸＋透過を持つ画像）を apple-touch-icon としても兼用していたため、iOSが要求する180x180へアップスケールされてぼやけて表示され、さらに透過部分の扱いにより縁が不自然に見える不具合があった。favicon.ico とは別に、180x180・透過なしの専用画像を新設して解消した。デザイン（配色・ローソク足モチーフ）自体は変更していない。
...
- block モードは画面表示と同一の列（コード・銘柄名・検出件数・最大売買代金・検出時刻・価格変化率・タイプ・日次売買代金）を返す。ただし金額列は「最大売買代金（円）」「日次売買代金（円）」とし、画面表示の億円表示ではなく円単位のヘッダ名にする（2026-07 追加）。
...
- block モードは r（/screening のレスポンス要素をそのまま使用。ピボット不要）から、コード・銘柄名・検出件数・最大売買代金（円のまま）・検出時刻・価格変化率・タイプ・日次売買代金（円のまま）を出力する（2026-07 追加）。
...
各要素にコード・銘柄名・検出件数・最大売買代金（円）・検出時刻（HH:MM）・価格変化率（％）・タイプ（値飛ばし/吸収）・日次売買代金（円）を含む配列。最大売買代金の降順でソートされる。
...
window.onload の loadTradingDates() → API /trading_dates → #blockDateSelect（1分足取得の制約上、選択肢を直近6営業日〔BLOCK_RECENT_TRADABLE_DAYS〕に絞り込んで構築）→ screening.js（startScreening() が #blockThresholdYen の入力値〔億円〕を円へ変換し threshold_yen として送信、#blockCandidateLimit をそのまま candidate_limit として送信、除外市場〔blockConditions〕を exclude_markets として送信）→ API /screening?mode=block（target_date・threshold_yen・candidate_limit・任意で exclude_markets）→ main.py が target_date を trading_dates_cache の先頭 BLOCK_RECENT_TRADABLE_DAYS 件と照合（範囲外ならエラー）→ 一次スクリーニング：ticker_list を走査し data.json の日次売買代金（出来高×終値）で降順ランキング、上位 candidate_limit 件に絞り込み（外部通信なし）→ 二次検査：候補銘柄のみ _fetch_block_detection（fetch_1m_bars による1分足取得＋detect_block_bars によるバー検出）を ThreadPoolExecutor で並列実行 → 検出バーがある銘柄のみ、検出件数・最大売買代金・検出時刻・価格変化率・タイプ（値飛ばし/吸収）・日次売買代金を結果に含めて返却（最大売買代金の降順でソート）→ showResults() でテーブル描画（金額は億円単位に整形して表示）→ #csvDownloadBtn クリック時は downloadCsv() が buildCsvHeaders/buildCsvRow で画面表示と同一列（金額は円のまま）のCSVを出力する
