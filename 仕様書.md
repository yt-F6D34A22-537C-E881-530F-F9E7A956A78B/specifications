...
- スコア（CSVスコア列がない場合は -）
- 上昇/下降の予測
- 比較元終値（固定列群の最右へ移動・固定列化。2行ヘッダ：1行目「比較元 YYYY/MM/DD（曜）」、2行目「終値」。2026-07 変更。旧仕様ではスコア・上昇/下降の予測・比較元終値は固定列ではなく、横スクロール領域に含まれていた）
...
screening_<mode>_<YYYYMMDD>.csv（YYYYMMDD はスクリーニングに指定した対象日。モードごとの日付セレクタ値〔ratioDateSelect / dateSelect / heuristicsDateSelect〕を使用する。compare モードは単一の対象日を持たないため screening_compare_<比較元日付>-<比較先日付>.csv とする。2026-07、compare モードのCSV出力を有効化した〔詳細は buildCsvHeaders/buildCsvRow・dataFlow.compareScreening を参照〕）
...
- compare モード時は2行ヘッダを生成する。1行目：固定列（コード・銘柄名・スコア・上昇/下降の予測は rowspan="2"、比較元終値は「比較元 ${compareFromLabel}」）＋ compareDateList の各日付を colspan="3" で1グループとして横展開したヘッダ（makeDateLabel(date) 形式）。2行目：比較元終値の「終値」＋ compareDateList の各グループぶんの「終値」「増減（円）」「増減（％）」。
- 比較元終値の1行目セルには data-fixed-col の代わりに data-fixed-col-group="4" を付与する（本体側の5番目＝インデックス4の固定列であることを明示し、syncFixedColumns() の左オフセット算出で対応付ける。詳細は fixedHeader・conventions.domHookAttributes を参照）。
...
- compare モードは画面表示（横持ち）と同一の列構成でヘッダを返す：固定列（コード・銘柄名・スコア・上昇/下降の予測・比較元終値）＋ compareDateList の比較先日付ごとに「YYYY/MM/DD（曜）終値」「YYYY/MM/DD（曜）増減（円）」「YYYY/MM/DD（曜）増減（％）」の3列（heuristics の「グループ名（item.label）」と同様、2行ヘッダ相当を1セルへ連結する方式）。「上昇/下降の結果」列は画面表示同様に出力しない（2026-07 変更）。
...
- compare モードは r として buildCompareWideRows() が生成した1銘柄1行の横持ちオブジェクトを受け取り、uploadedCsvScoreMap からスコアを取得し、固定列（コード・銘柄名・スコア・上昇/下降の予測・比較元終値）＋ compareDateList の比較先日付ごとの終値・増減（円）・増減（％）を出力する。「上昇/下降の結果」（calcCompareResult() の呼び出し）は画面表示同様に出力しない（2026-07 変更）。
...
- 各要素にコード・銘柄名・比較元終値・比較先終値・増減円・増減率・予測を含む配列（all_market_days 省略時。従来と同一構造）。
...
screening.js（currentResults。compare モードのみ buildCompareWideRows() で横持ちへ変換してから渡す）→ buildCsvHeaders / buildCsvRow / escapeCsvCell → Blob（BOM付きUTF-8）→ <a>.click() → ブラウザダウンロード（対応モードは ratio / date / heuristics / compare の4種。2026-07、compare モードのCSV出力を画面表示〔横持ち〕と同一の列構成へ修正し、有効化した）
