...
- データ源となる market_cap.json は scripts/fetch_market_cap.js・.github/workflows/market_cap.yml（週次実行）で生成する。発行済株式数は株式分割・自己株買い等がなければ短期間ではほぼ変化しないため、data.json（日次）ほど高頻度な更新は行わない設計とした。scripts/fetch_market_cap.js は当初 Python（yfinance）で実装していたが、他の既存スクリプト（fetch.js・margin.js等）と方式を統一するため Node.js（node-fetch で Yahoo Finance API を直接呼び出す方式）へ置き換えた（2026-07）。詳細は workflows.market_cap.yml を参照。
...
- 発行済株式数は株式分割・自己株買い等がなければ短期間ではほぼ変化しないため、data.json（日次更新）とは別に、低頻度（週次を想定）で更新されるデータソースとして設計している。生成は scripts/fetch_market_cap.js・.github/workflows/market_cap.yml を参照（2026-07、Python から Node.js へ置き換え。詳細は workflows.market_cap.yml を参照）。
...
- 0 21 * * 0（JST 月曜 06:00（UTC 日曜 21:00））。発行済株式数は株式分割・自己株買い等がなければ短期間ではほぼ変化しないため、data.json（fetch.yml・1日4回）や margin.json（margin.yml）ほど高頻度な更新は不要と判断し、週次とした
